...
namespace wServer.logic.loot
{
    ...
    public class Loot : List<ILootDef>
    {
            ...
            foreach (KeyValuePair<Player, IList<Item>> i in soulbound)
            {
                if (i.Value.Count > 0)
                {
                    ...
                    foreach (var l in i.Value)
                        if (l.BagType >= 6)
                            AnnounceLoot(enemy, l.ObjectId, i.Key.Name);
                }
                else
                    pub.Add(i.Key);
            }
        ...
        private static void AnnounceLoot(Enemy enemy, string name, string pname)
        {
            foreach (var i in enemy.Manager.Worlds.Values)
                foreach (var p in i.Players.Values)
                    p.SendInfo($"{pname} has obtained {name}, from {enemy.Owner.Name}");
        }
