    ...
    internal class VisitCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            foreach (KeyValuePair<string, Client> i in player.Manager.Clients)
                if (player.Client.Account.Rank <= 10 && i.Value.Player.Owner is PetYard || i.Value.Player.Owner is Vault) //This is the Rank thingy!
                {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            try
            {
                ...
                if (args.Length == 1)
                {
                    ...
                    player.Client.Save();
                    player.SendInfo("Your size has changed");
                }
            }
            catch
            {
...
internal class TossEffCommand : Command
    ...
    internal class TagCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            if (args.Length == 1)
            {
                if (player.Client.Account.Credits < 200)
                {
                    player.SendError("Not enough gold (200)");
                    return false;
                }
                if (player.Client.Account.Credits >= 200)
                {
    ...
    internal class SpawnCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            {
                player.SendInfo("Cannot spawn in nexus!");
                return false;
            }
    ...
    {
        public KillAll() : base("killAll", permLevel: 9) { }
