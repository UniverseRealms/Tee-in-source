        ...
        public bool Activate(RealmTime time, Item item, UseItemPacket pkt)
        {
            ...
            foreach (ActivateEffect eff in item.ActivateEffects)
            {
                switch (eff.Effect)
                {
                    ...
                    case ActivateEffects.MagicNovaSeal:
                    {
                            List<Packet> pkts = new List<Packet>();
                            this.Aoe(eff.Range / 2, true, player => { ActivateHealMp(player as Player, eff.Amount, pkts); });
                            pkts.Add(new ShowEffectPacket
                    ...
                    case ActivateEffects.LootBox:
                        {
                            ...
                            var lootpenis = eff.Rewards[Random.Next(0, eff.Rewards.Count)];
                            Inventory[pkt.SlotObject.SlotId] = Manager.GameData.Items[lootpenis];
                            Client.SendPacket(new CrateOpenPacket
                            {
                                ItemId = lootpenis
                            });
                            UpdateCount++;
