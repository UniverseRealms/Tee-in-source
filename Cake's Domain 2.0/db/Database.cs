...
AND characters.charId=death.chrId;";
        ...
        public Account Register(string uuid, string password, bool isGuest, XmlData data)
        {
            ...
            if (success)
            {
                cmd = CreateQuery();
                cmd.CommandText =
                    "INSERT INTO stats(accId, fame, totalFame, credits, totalCredits) VALUES(@accId, 0, 0, 49, 49);";
                cmd.Parameters.AddWithValue("@accId", accId);
                cmd.ExecuteNonQuery();
...
bestFame = GREATEST(bestFame, @bestFame);";
        ...
        public void StoreIP(string ip) //Adds users IP to table for itself to be.
        {
            bool IsThere = false;
            MySqlCommand cmd = CreateQuery();
            ...
            using (MySqlDataReader rdr = cmd.ExecuteReader())
            {
                while (rdr.Read())
                {
                    if (rdr.GetString("ip") == ip) //Ip is here, we do nothing.
                        IsThere = true;
                }
            }
        ...
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = "SELECT * FROM ips WHERE ip=@ipx;";
            cmd.Parameters.AddWithValue("@ipx", ip);

            using (MySqlDataReader rdr = cmd.ExecuteReader())
            {
                while (rdr.Read())
                {
                    if (rdr.GetInt16("banned") == 1) //If banned is 1, then user is ipbanned.
                        return true;
                }
            }
            return false;
        ...
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = "UPDATE ips SET banned=1 WHERE ip=@ipx;";
            cmd.Parameters.AddWithValue("@ipx", ip);
            cmd.ExecuteNonQuery();
        }
    }
