...
public class ActivateEffect
{
    public ActivateEffect(XElement elem)
    {
        ...
        if (elem.Attribute("rewards") != null)
        {
            Rewards = new List<ushort>();
            var rewards = elem.Attribute("rewards").Value;
            foreach (var txt in rewards.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries).Select(_ => _.Trim()))
                Rewards.Add((ushort)Utils.FromString(txt));
        }
    }
